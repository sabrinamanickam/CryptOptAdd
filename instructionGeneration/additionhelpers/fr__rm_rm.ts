...
import { ADX, isU1, zx } from "@/helper";
...
// a   a    1       coin: spillCF || spillOF //Spill CF and use adc
// z   z    2       coin: adox    || adcx //use adc
// a   k    3       zero OF + adox
// k   a    4       zero CF + adcx
// k   z    5       adox
// a   z    6       adox
...
/*function fr__rm_r(
  ...
  if (fs.CF == fs.OF) {
    ...
    if (fs[flagToUse] === FlagState.ALIVE) {
      ...
      /*const ra = RegisterAllocator.getInstance();
      const bytereg = ra.spillFlag(Flags.CF);
      if (!bytereg) {
