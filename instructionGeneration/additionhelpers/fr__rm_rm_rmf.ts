...
export function fr_rm_f_f(cout: string, out: string, arg0: U64Allocation): asm[] {
  ...
  return [
    `add ${reg}, ${arg0.store}; ${otherFlag} should have been spilled if it had deps, ${flag} should have been spilled into ${reg} and into another reg, if it has had other deps than this one.`,
  ];
}
...
function fr__r_rm_f(
  ...
  if (isMem(r1.store) && isU64(r1)) {
    ...
    return [`; in fr__r_rm_f`, `add ${r0store}, ${r1.store}`];
  }
  ...
  return [`; in fr__r_rm_f`, `add ${r1store}, ${r0store}`];
}
...
function fr__r_rm_rm(
  ...
  if (fs.CF !== FlagState.ALIVE && fs.OF !== FlagState.ALIVE) {
    ...
    res.push(`add ${rm64_4}, ${reg64_3}; loading flag`);
  }
  ...
  return res.concat(`add ${r0store}, ${r1store}`);
}
