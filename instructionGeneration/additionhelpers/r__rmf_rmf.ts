...
export function r__r_r(out: string, r0: RegisterAllocation, r1: RegisterAllocation): asm[] {
  ...
  const r1u1 = isU1(r1);
  if (r0u1 && r1u1) {
    return r__r1_r1(out, r0 as U1RegisterAllocation, r1 as U1RegisterAllocation); //done
  }
  if (r0u1 && !r1u1) {
    return r__r1_r64(out, r0 as U1RegisterAllocation, r1 as U64RegisterAllocation); //done
  }
  if (!r0u1 && r1u1) {
    return r__r1_r64(out, r1 as U1RegisterAllocation, r0 as U64RegisterAllocation);//done
  }
  if (!r0u1 && !r1u1) {
    return r__r64_r64(out, r0 as U64RegisterAllocation, r1 as U64RegisterAllocation);//done
  }
...
function r__rm_m(out: string, r0: RegisterAllocation | MemoryAllocation, m1: MemoryAllocation): asm[] {
  ...
  if (fs.CF !== FlagState.ALIVE && fs.OF !== FlagState.ALIVE) {
    ...
    if (!isByteRegister(r0store) && isU1(m1)) {
      ...
      return res.concat(`adc ${r0store}, 0x0`);  //adcExample
    }
...
function r__rm_f(out: string, r0: RegisterAllocation | MemoryAllocation, f1: U1FlagAllocation): asm[] {
  ...
  if (f1.store === Flags.CF) {
    ...
    if (fs.OF !== FlagState.ALIVE) {
      ...
      return [`adc ${r0store}, 0x0; add CF to r0's alloc`]; //adcExample
    } else {
      const reg = ra.loadImmToReg64("0x0");
      return [`add ${r0store}, ${reg}`];
    }
  } else {
    // OverflowFlag
    ...
    return res.concat(`add ${r0store}, ${readReg}`);//changed adox to add
  }
}
...
export function r__f_f(out: string): asm[] {
  ...
  if (cfDep && !ofDep) {
    ...
    // add OF to this
    return [inst, `add ${reg}, ${zero}; spilled cf, zxed it, + 0 + of`];//changed adox to add
  }
  ...
  if (!cfDep && ofDep) {
    ...
    return [
      ...
      `adc ${outreg}, 0x0; spilled of to r/8, zxed it, adc 0 to out`, //adcExample
    ];
  }
  ...
  ra.declareFlagState(Flags.OF, FlagState.ZERO);
  ra.declareFlagState(Flags.CF, FlagState.ZERO);
  return [`adc ${breg}, 0x0; r<-f+f`, zx(breg).inst]; //adcExample
}
